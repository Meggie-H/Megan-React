...
import { IBuildStats, IContributor, IContributorResponse, IIssue, ILanguage, ILanguageColors, ILanguageResponse, IWorkflowRuns, WorkflowRun } from '../models';
...
export async function getOpenIssueCount(owner: string, repo: string): Promise<number> {
  const data: IIssue[] = await makeApiRequest(`${baseUrl}${owner}/${repo}/issues?state=open`);
  return data.length;
}

export async function getClosedIssueCount(owner: string, repo: string): Promise<number> {
  const data: IIssue[] = await makeApiRequest(`${baseUrl}${owner}/${repo}/issues?state=closed`);
  return data.length;
}

export async function getBuildStats(owner: string, repo: string): Promise<IBuildStats> {
  const data: IWorkflowRuns = await makeApiRequest(`${baseUrl}${owner}/${repo}/actions/runs?status=completed&per_page=100`);

  const initialBuildStats: IBuildStats = { successes: 0, failures: 0 };
  const buildStats = data.workflow_runs.reduce((acc: IBuildStats, build: WorkflowRun) => {
...
export async function getContributors(owner: string, repo: string): Promise<IContributor[]> {
  const data: IContributorResponse[] = await makeApiRequest(`${baseUrl}${owner}/${repo}/contributors`);
...
export async function getLanguageStats(owner: string, repo: string): Promise<ILanguage> {
  const data: ILanguageResponse = await makeApiRequest(`${baseUrl}${owner}/${repo}/languages`);
